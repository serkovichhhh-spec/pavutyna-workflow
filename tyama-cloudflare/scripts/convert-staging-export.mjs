...
function eventTimestamp(eventId) {
  const event = eventById.get(String(eventId));
  return event?.updatedAt || event?.createdAt || '1970-01-01T00:00:00Z';
}
...
for (const h of hosts) {
  const timestamp = h.updatedAt || h.createdAt || '1970-01-01T00:00:00Z';
  lines.push(`INSERT OR IGNORE INTO hosts (id,email,display_name,created_at,updated_at) VALUES (${q(h.id)},${q(h.email)},${q(h.name || h.displayName || h.email)},${q(h.createdAt || timestamp)},${q(timestamp)});`);
}

for (const e of events) {
  const timestamp = e.updatedAt || e.createdAt || '1970-01-01T00:00:00Z';
  lines.push(`INSERT OR REPLACE INTO events (id,host_id,title,event_type,event_date,venue,hero_names,notes,lifecycle,questionnaire_token,public_screen_token,created_at,updated_at) VALUES (${q(e.id)},${q(e.hostId)},${q(e.title)},${q(e.type || 'Подія')},${q(e.date)},${q(e.venue)},${q(e.heroNames)},${q(e.notes)},${q(e.lifecycle || 'preparing')},${q(e.questionnaireToken)},${q(e.publicToken)},${q(e.createdAt || timestamp)},${q(timestamp)});`);
}

const questionToQuestionnaire = new Map();
const questionKey = new Map();
for (const form of questionnaires) {
  const timestamp = form.updatedAt || form.createdAt || eventTimestamp(form.eventId);
  lines.push(`INSERT OR REPLACE INTO questionnaires (id,event_id,title,intro,is_open,created_at,updated_at) VALUES (${q(form.id)},${q(form.eventId)},${q(form.title)},${q(form.intro)},${bool(form.isOpen !== false)},${q(form.createdAt || timestamp)},${q(timestamp)});`);
  (form.questions || []).forEach((question, index) => {
    ...
    lines.push(`INSERT OR REPLACE INTO questions (id,questionnaire_id,position,question_key,label,field_type,is_required,is_locked,privacy_default,options_json,created_at) VALUES (${q(question.id)},${q(form.id)},${index},${q(question.key)},${q(question.label)},${q(type)},${bool(question.required)},${bool(question.locked)},${q(safePrivacy(question.privacyDefault))},${q(JSON.stringify(question.options || []))},${q(question.createdAt || timestamp)});`);
  });
}

for (const r of responses) {
  const timestamp = r.submittedAt || eventTimestamp(r.eventId);
  const answers = Array.isArray(r.answers) ? r.answers : [];
  const consentAnswer = answers.find((a) => questionKey.get(String(a.questionId)) === 'consent');
  lines.push(`INSERT OR REPLACE INTO responses (id,event_id,questionnaire_id,respondent_label,consent,submitted_at) VALUES (${q(r.id)},${q(r.eventId)},${q(r.questionnaireId)},${q(r.respondentLabel || 'Гість')},${q(consentAnswer?.value ?? r.consent)},${q(timestamp)});`);
  for (const a of answers) {
    ...
    lines.push(`INSERT OR REPLACE INTO response_answers (id,response_id,question_id,answer_text,created_at) VALUES (${q(answerId)},${q(r.id)},${q(questionId)},${q(a.value ?? '')},${q(timestamp)});`);
  }
}

for (const item of kitItems) {
  const source = item.sourceResponseId && responseIds.has(String(item.sourceResponseId)) ? item.sourceResponseId : null;
  const timestamp = item.updatedAt || item.createdAt || eventTimestamp(item.eventId);
  lines.push(`INSERT OR REPLACE INTO kit_items (id,event_id,source_response_id,category,title,body,status,privacy,useful,edited,created_at,updated_at) VALUES (${q(item.id)},${q(item.eventId)},${q(source)},${q(item.category || 'stories')},${q(item.title || 'Матеріал')},${q(item.body || '')},${q(safeStatus(item.status))},${q(safePrivacy(item.privacy))},${bool(item.useful)},${bool(item.edited)},${q(item.createdAt || timestamp)},${q(timestamp)});`);
}
...
for (const rehearsalState of rehearsal) {
  ...
  const timestamp = rehearsalState.updatedAt || eventTimestamp(rehearsalState.eventId);
  for (const [itemId, value] of Object.entries(states)) {
    ...
    lines.push(`INSERT OR REPLACE INTO rehearsal_items (event_id,kit_item_id,state,updated_at) VALUES (${q(rehearsalState.eventId)},${q(itemId)},${q(stateValue)},${q(timestamp)});`);
  }
}

for (const entry of live) {
  const current = entry.currentItemId && kitIds.has(String(entry.currentItemId)) ? entry.currentItemId : null;
  const timestamp = entry.updatedAt || eventTimestamp(entry.eventId);
  lines.push(`INSERT OR REPLACE INTO live_state (event_id,current_item_id,updated_at) VALUES (${q(entry.eventId)},${q(current)},${q(timestamp)});`);
}

for (const event of events) {
  if (!live.some((l) => String(l.eventId) === String(event.id))) {
    lines.push(`INSERT OR IGNORE INTO live_state (event_id,current_item_id,updated_at) VALUES (${q(event.id)},NULL,${q(eventTimestamp(event.id))});`);
  }
}
